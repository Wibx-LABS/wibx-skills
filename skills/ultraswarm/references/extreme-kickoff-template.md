...
- About to touch a path outside your owned_paths, or to `git push` (it carries other fronts'
  commits)? Ask the manager first, append an ASK line, wait for the relayed answer.
...
5. <include only if depends_on is non-empty> Read contracts/<dep>.md for each upstream front
...
8. EXTREME GATE — before flipping status to done: run /code-review ultra on your branch,
...
the launcher already created the worktree and started you inside it, so replace step 4 with:
...
On each front's HELLO: record the message's `from` as that row's `owner`, then subscribe with
  notify_when_idle: true and NO message. The subscription is ONE-SHOT — re-subscribe on every
  notice or you go deaf to that front.
On an idle notice: re-read the blackboard. Under the DONE RULE, **idle is not done** — a front
  can go quiet with goal_met=no or reviewed=findings. Idle + not DONE = stalled; investigate.
  Note that /code-review ultra keeps a front BUSY for a long stretch; a long silence from a
  front in review is normal, an idle notice from one is not.
On a relay request (ASK): message the owning front, relay the answer back, append the GRANT.

Loop:
- Re-read your own name from the ListAgents header. Session names drift mid-run; if yours no
  longer matches the `manager:` line in blackboard.md, rewrite that line.
...
- You still cannot prevent a launch — but you CAN message a running downstream front and tell
  it to hold the shared edge until its feeder's contract is `stable`.

Never ask a worker to run something you were denied. Launched workers run with permission
checks off and no human at their window; they will comply, and the refusal is laundered.
