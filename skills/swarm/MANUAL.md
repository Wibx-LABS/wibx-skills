...
- **A Live Channel, Star-Shaped:** Files are the truth, but a file cannot answer a question or say "I'm stuck". So workers also message — and each one is given exactly **one** address, the manager's, and is forbidden to run `ListAgents`. Front-to-front is relayed by the manager; a worker never learns the machine's other sessions exist. Used for two things only: the handshake before a shared write or a push, and `notify_when_idle` liveness so a dead front is noticed instead of waited on.
...
- **Canal Vivo, em Estrela:** O arquivo é a verdade, mas arquivo não responde pergunta nem avisa "travei". Então os workers também se falam — e cada um recebe exatamente **um** endereço, o da gerente, e é proibido de rodar `ListAgents`. Front-a-front é relayado pela gerente; o worker nunca descobre que as outras sessões da máquina existem. Serve para duas coisas só: o handshake antes de uma escrita compartilhada ou de um push, e o sinal de vida `notify_when_idle`, para que um front morto seja notado em vez de esperado.
